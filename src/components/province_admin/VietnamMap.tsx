...
const VietnamMap = () => {
  ...
  const [locationsUpdated, setLocationsUpdated] = useState<number>(0);
...
{map && (
  <div >
    {clickedLayerGid !== null && (
      < LocationList provinceGid={clickedLayerGid} provinceName={clickedLayer} locationsVersion={locationsUpdated} onLocationDeleted ={()=> setLocationsDeleted(prev => prev +1)}
      />
    )}
    ...
    <LocationMarkers
      ...
      onLocationAdded={() => setLocationsUpdated(prev => prev + 1)}
      locationsVersionDeleted={locationsDeleted}
    />
  </div>
  )}
    </div>
