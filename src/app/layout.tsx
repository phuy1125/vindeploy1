...
import UserProfile from "./components/UserProfile"; // Import component UserProfile
            ...
            <nav className="w-full mx-auto relative z-10 flex items-center justify-between bg-white bg-black/80 py-0 text-black border-b border-blue-600/20 shadow-lg shadow-blue-400/20">
              ...
              <ul className="hidden text-lg md:flex w-1/2 items-center justify-between gap-4 py-2 cursor-pointer md:px-10 border-r-2 border-l-2 border-gray-800 dark:border-gray-300 group">
                ...
                <Link href="/" onClick={() => setActive('home')}>
                  <li className={`rounded-b px-2 ${
                    active === 'home' ? 'border-b-[3px] border-blue-600 font-bold' : 'hover:border-b-[3px] hover:border-blue-600 hover:font-bold group-hover:border-b-0'
                  }`}>
                    Trang chủ
                  </li>
                </Link>
                ...
                {/* Không gian chia sẻ */}
                <li onClick={() => setActive('share')} className={`rounded-b px-2 ${
                  active === 'share' ? 'border-b-[3px] border-blue-600 font-bold' : 'hover:border-b-[3px] hover:border-blue-600 hover:font-bold group-hover:border-b-0'
                }`}>
                  Không gian chia sẻ
                </li>
                ...
                <Link href="/aboutus" onClick={() => setActive('about')}>
                  <li className={`rounded-b px-2 ${
                    active === 'about' ? 'border-b-[3px] border-blue-600 font-bold' : 'hover:border-b-[3px] hover:border-blue-600 hover:font-bold group-hover:border-b-0'
                  }`}>
                    Về chúng tôi
                  </li>
                </Link>
              ...
              {/* Hiển thị avatar người dùng hoặc các nút Đăng nhập/Đăng ký */}
              <div className="hidden md:flex justify-start pl-2 pr-10 py-1 gap-12">
                <UserProfile />  {/* Hiển thị UserProfile để thay thế các nút Đăng nhập và Đăng ký */}
              </div>
            </nav>
          </header>
